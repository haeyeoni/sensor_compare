//
// Created by haeyeon on 20. 6. 4..
//

#include "count_point.h"


int distance;
double center_x;
double center_y;
double center_z;
double size;
typedef pcl::PointCloud<pcl::PointXYZ> PointCloud;
typedef pcl::PCLPointCloud2 PointCloud2;
typedef PointCloud::Ptr PointCloudPtr;
typedef PointCloud2::Ptr PointCloud2Ptr;

class CountPoints
{
public:
    CountPoints() {
        this->subscriber = this->nh.subscribe("/hps_camera/pointcloud_output", 10, &CountPoints::PointsInBounary, this);
        this->publisher = this->nh.advertise<sensor_msgs::PointCloud2> ("points_inside", 10);
        this->publisher2 = this->nh.advertise<std_msgs::Int32> ("num_points", 10);
    };

    void PointsInBounary(const sensor_msgs::PointCloud2 &cloud_msg) {
        pcl::PointXYZ origin(center_x, center_y, center_z);
        
        sensor_msgs::PointCloud2 points_out;
        std_msgs::Int32 num_points;

        PointCloudPtr cloud(new PointCloud); 
        PointCloud2Ptr cloud2(new PointCloud2);
        PointCloud2Ptr cloud_out(new PointCloud2);
        
        PointCloud result;
        pcl_conversions::toPCL(cloud_msg, *cloud2); // save cloud message to cloud2
        pcl::fromPCLPointCloud2(*cloud2, *cloud); 
        pcl::KdTree<pcl::PointXYZ>::Ptr tree_(new pcl::KdTreeFLANN<pcl::PointXYZ>);
        tree_->setInputCloud(cloud);

        std::vector<int> nn_indices(500000);
        std::vector<float> nn_dists(500000);

        tree_->nearestKSearch(origin, 500000, nn_indices, nn_dists);
        for (int i = 0; i < nn_indices.size(); i++) {
            bool inside_x = cloud->points[nn_indices[i]].x < center_x + size/2 && cloud->points[nn_indices[i]].x > center_x - size/2;
            bool inside_y = cloud->points[nn_indices[i]].y < center_y + size/2 && cloud->points[nn_indices[i]].y > center_y - size/2;
            bool inside_z = cloud->points[nn_indices[i]].z < center_z + size/2 && cloud->points[nn_indices[i]].z > center_z - size/2;
            if(inside_x && inside_y && inside_z) {
                result.push_back(cloud->points[nn_indices[i]]);
            }
            //result.push_back(cloud->points[nn_indices[i]]);
        }
        pcl::toROSMsg(result, points_out);
        points_out.header.frame_id = "hps_camera";
        num_points.data = result.size();
        //cout<< result.size();
        this->publisher.publish(points_out);
        this->publisher2.publish(num_points);
    }
    ~CountPoints(){
    }
    private:
        ros::NodeHandle nh;
        ros::Subscriber subscriber;
        ros::Publisher publisher;
        ros::Publisher publisher2;
};

int main(int argc, char **argv)
{
    ros::init(argc, argv, "talker");
    center_x = atof(argv[1]);
    center_y = atof(argv[2]);
    center_z = atof(argv[3]);
    size = atof(argv[4]);
    CountPoints CountPoints;
    while(ros::ok()) {
         ros::spinOnce();
    } 
    return 0;
}
